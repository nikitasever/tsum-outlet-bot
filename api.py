...
def _upsert_product(catalog: dict, sold: list, p: dict, now: int):
    ...
    Detect two types of sales:
    1. Whole product sold out (available: True → False)
    2. Individual size sold out (size.available: True → False)
    """
    url = p.get("url")
    if not url:
        return
...
async def scan_all_categories():
    ...
    while True:
        try:
            ...
            # HTML coming_soon scan (parallel)
            ...
            save_catalog(catalog)
            save_sold(sold[-5000:])
            logger.info(f"✅ Сканирование завершено за {round(time.time()-t_start,1)}s. "
                        f"Товаров: {len(catalog)}, продаж сегодня: "
                        f"{len([s for s in sold if s.get('ts',0) > now-86400])}")
...
async def coming_soon():
    ...
    return {"count": len(items), "items": items}    if prev and prev["product"].get("available") and not p.get("available") and not p.get("coming_soon"):
        sold.append({
            "product":    p,
            "ts":         now,
